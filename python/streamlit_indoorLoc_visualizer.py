...
st.title(':green[_Wi-Fi Fingerprinting Indoor Localization_]')
with st.sidebar:
    ...
    st.info('This application is for testing of trained models with direct interaction and result display')

# define global constants
project_home = '/Users/jackma/Documents/2YEAR4-S1/FYP/Web_interface/indoor_localization_prototype-main'
path_train = project_home + '/data/UJIIndoorLoc/trainingData2.csv'  # '-110' for the lack of AP.
path_validation = project_home + '/data/UJIIndoorLoc/validationData2.csv'  # ditto
path_model = project_home + '/results/scalable_indoor_localization_model'
N = 8
scaling = 0.2
training_ratio = 0.9
...
row_number = st.slider(':violet[Pick a Row from TestSet within range of 1110, the testing results will show below]', min_value=0, max_value=n_rows-1)
...
# Add a red point to the center of the plot
if row_number != 0:
...
fig.update_layout(
    scene=dict(
        ...
        zaxis=dict(range=[-1, 5], autorange=True, title="Floor"),
